...
const renderUsers = ()=>{
  ...
  return (
    <div className="App" style={{border:'5px solid black', margin:'10px', padding:'10px', borderRadius: '50px'}}>
      <h1 class='title'>User List</h1>
      <NewUser addUserCB={addUser}/>
      <button class={loading ? 'Loading':'Get Users'} disabled={loading} onClick={getUsers}>{loading ? 'Loading':'Get Users'}</button>
      <div>{renderUsers()}</div>
      <hr />
      <h1 class='title'>what our 'users' state looks like</h1>
      <div class='userString'>{JSON.stringify(users)}</div>
